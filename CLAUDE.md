...
- Scaffold Astro instalado: `^5.18.2` (Content Layer API estable). `src/content/config.ts`: `cases`/`metrics`/`siteCopy` usan `loader:` (`src/services/notionLoaders.ts`) y schema Zod alineado 1:1 con `docs/platform/notion-astro-contract.md`, con guardrail `superRefine` que bloquea el build si una ficha Insignia se publica sin Métrica ancla + Evidencia. Sin `NOTION_TOKEN` el build de producción (`astro build`/`astro sync`) falla a propósito; `.env` local gitignored, token lo administra Diego (recrear cuando falte); el mismo token vive como secret `NOTION_TOKEN` en el repo de GitHub para CI.
