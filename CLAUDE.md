...
- **Plantilla de caso (vigente desde 2026-08-11): CAR — Contexto → Acción → Resultado.** Reemplaza el esquema anterior de 8 secciones (Contexto/Problema/Mi rol/Sistema/Trade-offs/Resultados/Evidencia/Aprendizajes). Fuente canónica: "Plantilla v2 · Especificación de caso maestro" en Notion (`b50c60ba-af74-43da-90bc-09d31cf9d4c4`), reconciliada el mismo día que el spec/plan técnico. La sección Resultado usa una tabla `Métrica | Antes | Después` (se renderiza como tarjetas, no tabla). El cierre ("Reflexión") deja de ir en el body: es una propiedad Notion independiente (`Reflexión`, opcional). **Estado de implementación:** spec (`docs/superpowers/specs/2026-08-11-case-template-car-redesign-design.md`) y plan (`docs/superpowers/plans/2026-08-11-case-template-car-redesign.md`) escritos y en `master`; el código (`[slug].astro`, `case.css`, `config.ts`, `notionLoaders.ts`) y la propiedad `Reflexión` en Notion **todavía no existen** — no asumir que el rediseño ya está en producción hasta correr el plan. Solo la ficha SOFI tiene el contenido CAR ya validado (en el mockup de la sesión); las otras 14 fichas publicadas siguen con la estructura vieja de 8 secciones (tarea de reescritura: Notion `3b90fe3c-51c5-814f-8db0-d551e3414752`). Logro = Verbo + qué + cómo + impacto + timeframe (sigue vigente, no cambia con CAR).
...
- **Gotcha detectado 2026-08-12:** la parte `htmlhint` del mismo script `lint` apunta a rutas de **raíz** del repo (`index.html`, `portfolio/index.html`, los 4 casos legacy, `politicas-privacidad.html`, `terminos-y-condiciones.html`, `404.html`) — no a `public/` ni a `src/pages/*.astro`. La mayoría de esas rutas de raíz son código muerto (ver tabla de arquitectura §3), así que `npm run lint` no valida nada de lo que realmente se sirve en producción. Tarea de backlog abierta para redefinir el scope: Notion `3ba0fe3c-51c5-8124-833f-d512d01dba5f`. No asumir que un `npm run lint` limpio certifica el HTML/Astro live.
