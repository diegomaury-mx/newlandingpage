...
- `src/pages/index.astro` lee el singleton `siteCopy` (parseado por `src/utils/parseSiteCopy.ts`, que divide el markdown plano por headings `S<n> · <nombre>`/`SEO`) y renderiza S1-S8 + Footer con el copy canónico. S7b se omite a propósito (su copy declara "sin copy propio, lo resuelve el widget" de Senja, aún pendiente de activar).
